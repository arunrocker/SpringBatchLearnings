...
//	@Bean
	...
	private Step secondStep() {
		return steps.get("Second Step").<StudentJdbc,StudentJdbc>chunk(3)
				.reader(jdbcItemReader())//.processor(itemProcessor)
	...
	public JdbcCursorItemReader<StudentJdbc> jdbcItemReader() {
		JdbcCursorItemReader<StudentJdbc> studentJdbc = new JdbcCursorItemReader<>();
		studentJdbc.setDataSource(universitydatasource());
		studentJdbc.setSql("select id,first_name,last_name,email from student");
		BeanPropertyRowMapper<StudentJdbc>rowMapper = new BeanPropertyRowMapper<>();
		rowMapper.setMappedClass(StudentJdbc.class);
		studentJdbc.setRowMapper(rowMapper);
		studentJdbc.setMaxItemCount(8);
		studentJdbc.setCurrentItemCount(3);
		return studentJdbc;
	}
